...
from rna import db, login_manager
...
class User(db.Model, UserMixin):
    ...
    # it is like creating an author column on Run model - sysxetisi pinakwn
...
class Run(db.Model):
    ...
    image_file = db.Column(db.String(20), nullable=False, default="default-rna.png")
    # result = db.Column(db.String(100), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)

    def __repr__(self):
        return f"Run('{self.sequence}', '{self.date_time}', '{self.result}')"
